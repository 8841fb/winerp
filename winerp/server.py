...
from .lib.payload import MessagePayload, Payloads
...
class Server:
        ...
        for cid, each_client in self.active_clients.items():
            if each_client["id"] == client["address"][1]:
                ...
                if cid in self.on_hold_connections:
                    logger.info(
                        "On Hold Client moved to active client with connection id %s and local id %s",
                        self.on_hold_connections[cid]["id"],
                        cid,
                    )
    ...
    def __send_message(self, client, message):
        ...
        self.websocket.send_message(client, orjson.dumps(message))

    def __send_error(self, client, payload):
        if not isinstance(payload, dict):
            payload = payload.to_dict()

        self.websocket.send_message(client, orjson.dumps(payload))
    ...
    def __on_message(self, client, _, msg):
        ...
        if msg.type.verification:
            if msg.id in self.active_clients:
                logger.info(
                    "Connection from duplicate client has benn put on hold connection id %s and local id %s",
                    client["address"][1],
                    msg.id,
                )
                ...
                logger.info(
                    "Client verified with connection id %s and local id %s",
                    client["address"][1],
                    msg.id,
                )
                self.active_clients[msg.id] = {
                    "client": client,
                    "id": client["address"][1],
                }
                del self.pending_verification[client["address"][1]]
        ...
        if msg.type.information:
            logger.debug(
                "Received Information Message from client %s", client["address"][1]
            )
        ...
        if msg.type.ping:
            ...
            if (
                msg.destination is not None and msg.destination in self.active_clients
            ) or msg.destination is None:
        ...
        if msg.type.request:
            logger.debug(
                "Received Request Message from client %s", client["address"][1]
            )
            if msg.id == msg.destination:
                ...
                self.__send_message(
                    self.active_clients[msg.destination]["client"], payload
                )
                logger.debug(
                    "Request Message Forwarded to %s",
                    self.active_clients[msg.destination]["client"]["address"][1],
                )

        if msg.type.response or msg.type.error or msg.type.function_call:
            logger.debug(
                "Received Response Message from client %s", client["address"][1]
            )
            ...
            self.__send_message(self.active_clients[msg.destination]["client"], payload)
            logger.debug(
                "Response forwarded to %s",
                self.active_clients[msg.destination]["client"]["address"][1],
            )
